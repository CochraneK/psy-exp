...
(function(root,factory){
  ...
  function recordAttempt({sessionId,participantId,testKey,taskVersion,protocolId,qcStatus,resultRecordedAt,source='participant-manager'}={}){
    const d=load(),sid=String(sessionId||''),pid=safeId(participantId),attempt={id:uid('attempt'),studyId:d.study.id,siteId:(d.sessions[sid]&&d.sessions[sid].siteId)||DEFAULT_SITE_ID,sessionId:sid||null,participantId:pid,testKey:safeId(testKey),taskVersion:taskVersion||null,protocolId:protocolId||null,qcStatus:qcStatus||'unverified',recordedAt:resultRecordedAt||now(),source};d.attempts.push(attempt);audit(d,'attempt_recorded',{attemptId:attempt.id,sessionId:attempt.sessionId,participantId:pid,testKey:attempt.testKey,qcStatus:attempt.qcStatus});save(d);return clone(attempt)}
  ...
  function exportBundle(extra={}){const d=load();return{exportedAt:now(),format:'psy-exp-research-data-bundle',modelVersion:MODEL_VERSION,researchData:d,participantData:clone(extra.participantData)||null}}
  ...
  return{MODEL_VERSION,STORAGE_KEY,DEFAULT_STUDY_ID,DEFAULT_SITE_ID,configureStudy,ensureParticipant,beginSession,getActiveSession,updateSession,closeSession,recordAttempt,registerBuild,snapshot,exportBundle,downloadBundle,_load:load,_save:save,_clear:storage.clear};
});

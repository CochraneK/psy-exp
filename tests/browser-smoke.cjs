...
async function main() {
  ...
  try {
    ...
    await test('research-boundary homepage renders and current participant runtime remains usable', async()=>{
      ...
      await waitFor(`document.documentElement.getAttribute('data-psy-core')==='ready' && window.PSY_EXP_ORIGINAL_UI_CORE_READY===true && window.PSY_EXP_ORIGINAL_UI_MODERN_CORE==='1.0.0'`,8000);

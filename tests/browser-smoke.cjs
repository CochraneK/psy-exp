...
async function main() {
  ...
  try {
    ...
    await test('research-boundary homepage renders and current participant runtime remains usable', async()=>{
      ...
      const modalA11y=await evaluate(`(()=>{const opener=[...document.querySelectorAll('button')].find(b=>b.textContent.includes('被试管理'));opener.focus();opener.click();const dlg=document.getElementById('participantManagerOverlay'),close=document.getElementById('participantManagerClose');const opened=dlg.getAttribute('role')==='dialog'&&dlg.getAttribute('aria-modal')==='true'&&document.activeElement===close;document.dispatchEvent(new KeyboardEvent('keydown',{key:'Escape',bubbles:true}));return opened&&dlg.style.display==='none'&&document.activeElement===opener})()`);
      if(!modalA11y)throw new Error('participant manager dialog focus/Escape contract failed');

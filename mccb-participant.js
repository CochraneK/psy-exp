...
const TASK_TIMING_POLICY={
  ...
  bacs:{mode:'absolute-deadline'},fluency:{mode:'absolute-deadline'},hvlt:{mode:'absolute-deadline'},
  cpt:{mode:'absolute-trial-schedule-with-onset-log'},
  'spatial-span':{mode:'absolute-deadline'},lns:{mode:'absolute-deadline'},bvmt:{mode:'absolute-deadline'},
  mazes:{mode:'monotonic-elapsed'},msceit:{mode:'monotonic-elapsed'}
};
...
const _ls={get(k){try{return localStorage.getItem(k)}catch{return null}},set(k,v){try{localStorage.setItem(k,v);return true}catch(e){console.warn('localStorage 写入失败:',k,e&&e.message||e);return false}},remove(k){try{localStorage.removeItem(k);return true}catch{return false}},json(k){try{const v=this.get(k);return v?JSON.parse(v):null}catch{return null}},length(){try{return localStorage.length}catch{return 0}},key(i){try{return localStorage.key(i)}catch{return null}}};
...
const ExperimentRuntime=(()=>{
  ...
  const ensure=k=>{if(!VALID_TEST_KEYS.has(k))return null;if(!sessions.has(k))sessions.set(k,make(k));return sessions.get(k)};
...
const ParticipantManager={
  ...
  getTestUrl(k,mode){const t=TEST_ORDER.find(x=>x.key===k);if(!t)return'#';const p=new URLSearchParams();p.set('mode',mode||'user');const id=this.getCurrent();if(isValidCohortId(id))p.set('p',id);return t.file+'?'+p.toString()},

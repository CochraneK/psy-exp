...
const PARTICIPANT_SCHEMA_VERSION=3;
const RUNTIME_VERSION='research-runtime-0.3.0';
const DEFAULT_TASK_VERSION='research-web-0.3.0';
const TASK_VERSIONS={tmt:'tmt-web-0.3.0',bacs:'bacs-web-0.3.0',fluency:'fluency-web-0.3.0',cpt:'cpt-web-0.3.0','spatial-span':'spatial-span-web-0.3.0',lns:'lns-web-0.3.0',hvlt:'hvlt-web-0.3.0',bvmt:'bvmt-web-0.3.0',mazes:'mazes-web-0.3.0',msceit:'msceit-web-0.3.0'};
const TASK_TIMING_POLICY={
  bacs:{mode:'absolute-second-interval',nominalMs:1000},
  fluency:{mode:'absolute-second-interval',nominalMs:1000},
  hvlt:{mode:'absolute-second-interval',nominalMs:1000},
  cpt:{mode:'instrumented-timeout',nominalMs:[400,700]},
  tmt:{mode:'monotonic-elapsed'},
  'spatial-span':{mode:'legacy-scheduled'},lns:{mode:'legacy-scheduled'},bvmt:{mode:'legacy-scheduled'},mazes:{mode:'legacy-scheduled'},msceit:{mode:'legacy-scheduled'},
};
...
function taskVersion(testKey){return TASK_VERSIONS[testKey]||DEFAULT_TASK_VERSION}
...
const ExperimentRuntime=(()=>{
  ...
  function event(testKey,type,detail){const s=ensure(testKey);if(!s)return null;if(s.events.length>=1200)s.events.shift();s.events.push({type,at:new Date().toISOString(),elapsedMs:Math.max(0,Math.round(mono()-s.startedPerfMs)),detail:detail||null});return snapshot(testKey)}
  ...
  function recordTiming(testKey,label,plannedAtMs,actualAtMs,detail){const s=ensure(testKey);if(!s)return null;const driftMs=Math.max(0,actualAtMs-plannedAtMs);s.qc.timerSamples++;s.qc.timerDriftMaxMs=Math.max(s.qc.timerDriftMaxMs,Math.round(driftMs));event(testKey,'timing_sample',{label,plannedAtMs:Math.round(plannedAtMs),actualAtMs:Math.round(actualAtMs),driftMs:Math.round(driftMs),...(detail||{})});const nominal=detail&&Number(detail.nominalMs);if(Number.isFinite(nominal)&&nominal>0&&driftMs>nominal)invalidate(testKey,'timing_violation',`${label}_drift_exceeded_nominal_interval`);return snapshot(testKey)}
  function complete(testKey){const s=ensure(testKey);if(!s)return{testKey,taskVersion:taskVersion(testKey),runtimeVersion:RUNTIME_VERSION,status:'unverified',startedAt:null,completedAt:new Date().toISOString(),elapsedMs:null,clock:null,qc:{visibilityInterruptions:0,timingViolation:false,technicalFailure:false,timerDriftMaxMs:0,timerSamples:0,reasons:['session_not_started_through_runtime']},events:[]};if(s.completedAt==null){s.completedAt=new Date().toISOString();s.elapsedMs=Math.max(0,Math.round(mono()-s.startedPerfMs));event(testKey,'complete',{finalStatus:s.status})}return snapshot(testKey)}
  function getActiveTestKeys(){return Array.from(sessions.entries()).filter(([,s])=>s&&s.completedAt==null).map(([k])=>k)}
  function deadline(durationMs,{testKey,label='deadline',onTick,onDone,tickMs=100}={}){const startAt=mono(),end=startAt+Math.max(0,Number(durationMs)||0);let timer=null,cancelled=false;if(testKey)event(testKey,'deadline_start',{label,durationMs:Number(durationMs)||0});function step(){if(cancelled)return;const now=mono(),remainingMs=Math.max(0,end-now);if(typeof onTick==='function')onTick({elapsedMs:now-startAt,remainingMs,deadlineMs:end});if(remainingMs<=0){if(testKey)recordTiming(testKey,label,end,now,{nominalMs:Number(durationMs)||0,kind:'deadline_completion'});if(typeof onDone==='function')onDone({elapsedMs:now-startAt,overshootMs:Math.max(0,now-end)});return}timer=setTimeout(step,Math.min(tickMs,remainingMs))}step();return{cancel(){cancelled=true;if(timer)clearTimeout(timer)},startedPerfMs:startAt,deadlinePerfMs:end}}
...
function installTimerGuards(){
  ...
  const key=/mccb-bacs\.html$/.test(path)?'bacs':/mccb-fluency\.html$/.test(path)?'fluency':/mccb-hvlt\.html$/.test(path)?'hvlt':/mccb-cpt\.html$/.test(path)?'cpt':null;
  if(!key)return;
  ...
  if(key==='cpt'){
    window.setTimeout=function(fn,delay,...args){const d=Number(delay);if(typeof fn!=='function'||(d!==400&&d!==700))return nativeSetTimeout(fn,delay,...args);const planned=mono()+d;return nativeSetTimeout(function(){const actual=mono();if(ExperimentRuntime.snapshot('cpt'))ExperimentRuntime.recordTiming('cpt',d===400?'cpt_stimulus_window':'cpt_iti',planned,actual,{nominalMs:d,kind:'timeout_callback'});return fn(...args)},d)};
    window.clearTimeout=function(id){return nativeClearTimeout(id)};
    return;
  }
  let seq=-1;const custom=new Map();
  window.setInterval=function(fn,delay,...args){const d=Number(delay);if(typeof fn!=='function'||d!==1000)return nativeSetInterval(fn,delay,...args);const id=seq--;const startAt=mono();const state={cancelled:false,timer:null,ticks:0};custom.set(id,state);function step(){if(state.cancelled)return;const now=mono();const due=Math.floor((now-startAt)/d);while(!state.cancelled&&state.ticks<due){state.ticks++;const planned=startAt+state.ticks*d;ExperimentRuntime.recordTiming(key,`${key}_second_tick`,planned,now,{nominalMs:d,kind:'absolute_interval_tick',tick:state.ticks});fn(...args)}if(state.cancelled)return;const next=startAt+(state.ticks+1)*d;state.timer=nativeSetTimeout(step,Math.max(0,next-mono()))}state.timer=nativeSetTimeout(step,d);return id};
  window.clearInterval=function(id){if(custom.has(id)){const state=custom.get(id);state.cancelled=true;if(state.timer!=null)nativeClearTimeout(state.timer);custom.delete(id);return}return nativeClearInterval(id)};
}
...
function installRuntimeGuards(){
  ...
  if(typeof window!=='undefined'){
    window.ParticipantManager=ParticipantManager;window.ExperimentRuntime=ExperimentRuntime;window.PSY_EXP_BUILD=ParticipantManager.getBuildInfo();
...
if(typeof module!=='undefined'&&module.exports)module.exports={ParticipantManager,ExperimentRuntime,TEST_ORDER,KEY_TO_RESULT,RESULT_TO_KEY,RESULT_SCHEMA_VERSION,PARTICIPANT_SCHEMA_VERSION,RUNTIME_VERSION,TASK_VERSIONS,TASK_TIMING_POLICY,isValidCohortId};